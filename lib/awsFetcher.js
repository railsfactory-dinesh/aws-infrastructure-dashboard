...
function runAwsCli(cmd, profile = 'default', region = 'us-east-1', timeoutMs = 15000) {
  ...
  let fullCmd = `aws ${cmd} --region ${region}`;
  
  // If environment variables or IAM role is active, or profile is default / env / iam-role
  const isEnvOrRole = Boolean(process.env.AWS_ACCESS_KEY_ID) || profile === 'iam-role' || profile === 'env';
  
  if (profile && profile !== 'default' && !isEnvOrRole) {
