...
export function loadCredentialsFromSecretsManager(secretName = process.env.AWS_SECRET_NAME, region = 'us-east-1') {
  ...
  try {
    ...
    const stdout = execSync(`aws secretsmanager get-secret-value --secret-id ${secretName} --region ${region} --no-cli-pager`, {
      encoding: 'utf8',
      timeout: 10000,
...
/**
 * Fetches complete infrastructure details via AWS CLI safely
 */
export function fetchInfrastructureData(profile = 'default', region = 'us-east-1') {
  ...
  // 1. ECS Clusters (Safely isolated)
  try {
    ...
    console.warn('[ECS Fetch Warning]:', ecsErr.message);
  }

  // 2. EC2 Instances (Safely isolated)
  let ec2Instances = [];
  ...
  try {
    ...
    console.warn('[EC2 Fetch Warning]:', ec2Err.message);
  }

  // 3. RDS DB Instances (Safely isolated)
  let rdsInstances = [];
  ...
  const totalServices = clusters.reduce((acc, c) => acc + (c.services ? c.services.length : 0), 0);
  ...
  return {
    ...
    // Wrapped under keys that match what App.jsx reads: data.ecs, data.ec2, data.rds, data.s3
    ecs: {
      clusters,
      totalClusters: clusters.length,
      totalServices,
    ...
    // Also keep flat keys for backward compatibility
    clusters,
    ec2Instances,
    rdsInstances,
    s3Buckets
